	...
	en.OnPrefix("随机图片").Limit(ctxext.LimitByUser).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			if args == "" {
				...
				url := json.Get("data.0.urls.regular").Str
				if url == "" {
					ctx.SendChain(message.Text("什么都没有搜到噢，换一个关键词罢~"))
					return
				}
				url = strings.ReplaceAll(url, "i.pixiv.cat", "i.pixiv.re")
				...
				// 	process.SleepAbout1sTo2s()
				// 	if id := ctx.Send(message.Message{ctxext.FakeSenderForwardNode(ctx, message.Image(url).Add("cache", "0"))}).ID(); id == 0 {
				// 		ctx.SendChain(message.Text("ERROR: 可能被风控或下载图片用时过长，请耐心等待"))
				// 	}
				// }
			}
